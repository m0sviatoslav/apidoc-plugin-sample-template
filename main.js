...
require([
    ...
    Handlebars.registerHelper('if_eqa', function(a, b, opts) {
        if (a == b) {
            return opts.fn(this);
        } else {
    ...
    function changeVersionCompareTo(e) {
        ...
        if (compareVersion && articleVersions[group][name][0] === selectedVersion || version === selectedVersion) {
            ...
            if (entry.parameter && entry.parameter.samples)
                fields._hasTypeInParameterFields = _hasTypeInFields(entry.parameter.samples);
            ...
            if (fields._hasTypeInParameterFields !== true && entry.parameter && entry.parameter.samples)
                fields._hasTypeInParameterFields = _hasTypeInFields(entry.parameter.samples);
    ...
    function addArticleSettings(fields, entry) {
        ...
        if (entry.header && entry.header.samples) {
            sortFields(entry.header.samples);
            fields._hasTypeInHeaderFields = _hasTypeInFields(entry.header.samples);
        }

        if (entry.parameter && entry.parameter.samples) {
            sortFields(entry.parameter.samples);
            fields._hasTypeInParameterFields = _hasTypeInFields(entry.parameter.samples);
        }
